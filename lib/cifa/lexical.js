...
const DFAInfo = require('./DFAInfo.json')
...
const lexicalCompile = (config, source) => {
    ...
    while (lexicalBegin < sourceArr.length) {
        ...
        if (tempRes.type === '95') {
            // 注释中的换行
            line += tempRes.buffArr.filter(x => x === '\n').length
        }
        // 匹配到类型2, 即换行符, 将行数加一
        if (tempRes.type === "2") 
            ++line;
        ...
        // 类型对应的信息
        tempRes.info = DFAInfo[tempRes.type]
        // 保存当前匹配信息
        allArr.push(tempRes)
        if (tempRes.msg)
            msgArr.push(tempRes)
    ...
    // print(resArr)
    return {returnRes, returnMsg, resArr, msgArr, allArr, DFAInfo}
}
