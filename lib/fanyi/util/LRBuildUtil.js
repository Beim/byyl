...
/**
 * LR1 的CLOSURE计算, 考虑推导为空的情况
 * @param grammar Object
 * @param first Object
 * @param origin_itemSet Array
 * @return Array
 * 不会改变原来itemSet的值
 */
const closure_1_nil = (grammar, first, origin_itemSet) => {
    let itemSet = copy(origin_itemSet)
    let itemFirst = item_first(first)
    ...
    while (hasChange) {
        ...
        for (let i in itemSet) {
            ...
            for (let production of procs) {
                production.right.idx = 0
                let bs = item.right.val.slice(idx + 1).concat(item.next)
                // first(bs)
                let termiSymbols = itemFirst(bs)
                // let termiSymbols = itemFirst(item.right.val.slice(idx + 1).concat(item.next))
                for (let termiSymbol of termiSymbols) {
                    let production_copy = copy(production)
                    production_copy.next = termiSymbol
                    // B -> .r, k 是否在itemSet 中
 ...
 * @return Array 经过GOTO 计算的项集
 */
const goto_1 = (grammar, first, itemSet, symbol, closure = closure_1) => {
    let nextItemSet = []
    for (let item of itemSet) {
        if (item.right.idx < item.right.val.length 
            && item.right.val[item.right.idx] === symbol) {
    ...
    return closure(grammar, first, nextItemSet)
}

/**
 * LR1 的GOTO计算, 考虑推导出空的情况
 * @param grammar Object LR1 的表达式文法
 * @param first Object first集
 * @param itemSet Array 项集
 * @param symbol String 文法符号
 * @return Array 经过GOTO 计算的项集
 */
const goto_1_nil = (grammar, first, itemSet, symbol) => {
    return goto_1(grammar, first, itemSet, symbol, closure_1_nil)
}
...
/*
 * 计算first 集合, 包括推导出空的情况
 * @param grammar Object
 * @param terminators Array
 * @param nonTerminators Array
 * @return Object
 */
const first_nil = (grammar, terminators, nonTerminators) => {
    let first = {}
    for (let i of terminators) first[i] = [i]
    for (let i of nonTerminators) first[i] = []
    ...
    while (hasChange) {
        ...
        for (let X of nonTerminators) {
            let items = hasLeft(grammar, X)
            for (let item of items) {
                let vals = item.right.val
                for (let val of vals) {
                    let changed = combine(first[X], first[val])
                    if (changed) hasChange = true
                    if (!first[val].includes('nil')) break
                }
            }
        }
    }
...
/*
 * 计算follow集, 不考虑有推导为空的情况
 * @param grammar Object
 * @param first Object
 * @param nonTerminators
 */
...
/*
 * 给定一个文法的first集, 返回items 的first集
 * @param first Object
 * @return Function
 *      @param items Array
 *      @return Array
 */
const item_first = (first) => {
    return (items) => {
        let res = []
        for (let item of items) {
            let firstArr = first[item]
            if (!firstArr.includes('nil')) 
                res = res.filter(x => x !== 'nil')
            combine(res, firstArr)
            if (!firstArr.includes('nil')) break
        }
        return res
    }
}
 ...
 * @param nonTerminators
 */
const follow_nil = (grammar, first, nonTerminators) => {
    let follow = {}
    let itemFirst = item_first(first)
    for (let i of nonTerminators) follow[i] = []
    let startSymbol = grammar[0].left.val
    ...
    while (hasChange) {
        ...
        for (let i in grammar) {
            ...
            for (let j in right) {
                ...
                if (isNonterminator(right[j])) {
                    if (j < (right.length - 1)) {
                        let followedFirst = itemFirst(right.slice(j + 1))
                        changed = combine(follow[right[j]], followedFirst.filter(x => x !== 'nil'))
                        if (changed) hasChange = true
                        if (followedFirst.includes('nil')) {
                            changed = combine(follow[right[j]], follow[left])
                            if (changed) hasChange = true
                        }
                    } else if (j === (right.length - 1)) {
                        changed = combine(follow[right[j]], follow[left])
 ...
 * @return Object{Array, Object}
 */
const buildLR1_nil = (grammar, terminators, nonTerminators, firstOne = 0) => {
    // first 集
    let FIRST = first_nil(grammar, terminators, nonTerminators)
    // 所有的文法符号
    let symbols = terminators.concat(nonTerminators)
    // 开始的文法表达式对应的项
    let firstItem = copy(grammar[firstOne])
    ...
    firstItem.next = '$'
    // 项集族
    let itemFamily = [ closure_1_nil(grammar, FIRST, [firstItem]) ]
    ...
    while (hasChange) {
        ...
        for (let i in itemFamily) {
            for (let symbol of symbols) {
                let nextItemSet = goto_1_nil(grammar, FIRST, itemFamily[i], symbol)
                if (nextItemSet && nextItemSet.length > 0 ) {
                    if (!hasItemSet(itemFamily, nextItemSet)) {
                        itemFamily.push(nextItemSet)
                        hasChange = true
...
const buildLRTable_1 = (itemFamily, gotos, grammar, terminators, nonTerminators) => {
    ...
    for (let i in itemFamily) {
        ...
        for (let terminator of terminators) {
            ...
            if (time > 1) {
                conflicts.flag = -1
                conflicts.msg += `###conflict! ACTION[${i}][${terminator}]\n`
                for (let key in itemFamily[i]) {
                    let value = copy(itemFamily[i][key])
                    let left = value.left.val
                    let right = value.right.val
                    right.splice(value.right.idx, 0, '.')
                    conflicts.msg += `${key} : ${left} -> ${right.join(' ')} -> ${value.next}\n`
                }
            }
        }
    ...
    if (conflicts.flag < 0) return conflicts
    return {
        flag: 1,
        ACTION,
        GOTO
...
module.exports = {
    ...
    follow, 
    item_first,
    follow_nil, // 计算follow集
    buildLR0,
    buildLR1,
    buildLR1_nil, // 构造LR1自动机
    displayLR1ItemFamily,
    filter,
    buildLRTable_1, // 构造ACTION， GOTO表
}
