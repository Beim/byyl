    ...
    for (let i in codeStack) {
        ...
        // 没有arg2
            ...
            if (op === '=') {
                ...
                // ifFalse a < b goto 101
                push(`${i}:  ifFalse ${arg1.left.id} ${arg1.op.id} ${arg1.right.id} goto ${result.id}`)
...
const printCodeStack_4 = (codeStack) => {
    let resArr = []
    const push = (str) => resArr.push(str)
    for (let i in codeStack) {
        let item = codeStack[i]
        ...
        if (arg2 !== undefined) {
            ...
            if (op === '=()') {
                // x = (float) y
                // push(`${i}:  ${result.id} = (${arg1}) ${arg2.id}`)
                push(`( =(), ${arg1}, ${arg2.id}, ${result.id} )`)
            } else if (op === 'call') {
                // temp = call f, 1
                // push(`${i}:  ${result.id} = call ${arg1}, ${arg2}`)
                push(`( call, ${arg1}, ${arg2}, ${result.id} )`)

            } else {
                // x = y + z
                // push(`${i}:  ${result.id} = ${arg1.id} ${op} ${arg2.id}`)
                push(`( ${op}, ${arg1.id}, ${arg2.id}, ${result.id} )`)
            }
        // 没有arg2
        } else {
            if (op === '=') {
                ...
                // x[i] = y
                // push(`${i}:  ${base.id}[${offset.id}] = ${arg1.id}`)
                push(`( []=, ${arg1.id}, ${offset.id}, ${base.id} )`)

            } else if (op === '=[]') {
                let {base, offset} = arg1
                // x = y[i]
                // push(`${i}:  ${result.id} = ${base.id}[${offset.id}]`)
                push(`( =[], ${offset.id}, ${base.id}, ${result.id} )`)
                ...
                // if a < b goto 101
                // push(`${i}:  if ${arg1.left.id} ${arg1.op.id} ${arg1.right.id} goto ${result.id}`)
                push(`( j${arg1.op.id}, ${arg1.left.id}, ${arg1.right.id}, ${result.id} )`)
                ...
                let oop = trans[arg1.op.id]
                if (oop) {
                    push(`( j${oop}, ${arg1.left.id}, ${arg1.right.id}, ${result.id} )`)
                }

            } else if (op === 'return') {
                // return x;
                // push(`${i}:  return ${result.id}`)
                push(`( return, -, -, ${result.id} )`)

            } else if (op === 'param') {
                // param 1
                // push(`${i}:  param ${result.id}`)
                push(`( param, -, -, ${result.id} )`)
