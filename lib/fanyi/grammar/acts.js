...
module.exports = (acType) => {
    ...
    return (s_global, stack, symbols) => {
        ...
        const mergelist = (...args) => args.reduce((prev, curr) => prev.concat(curr))
        ...
        */
        ...
        const actObj = {
            ...
            // 'T3-242': () => {
            //     let top = s_global['top']
            //     let offset = s_global['offset']
            //     let Env = s_global['Env']
            //     let Stack = s_global['Stack']

            //     // 恢复保存的符号栈和偏移量, 并将record中的符号栈和偏移量返回
            //     let type = record(top)
            //     let width = offset
            //     s_global['top'] = Env.pop()
            //     s_global['offset'] = Stack.pop()
            //     return {type, width}
            // },
            // // T -> record { nT2 D } -> T3-242
            // // nT2 -> nil -> nT2-242
